    ...
    fun listVoices(): List<Voice> {
        ...
        while (true) {
            val result = runCatching {
                val pointer = arrayPointer?.getPointer(id * ptrSize)
                ...
                val gender = pointer?.getByte((ptrSize * 3))?.toUByte() ?: 0u
                val age = pointer?.getByte((ptrSize * 3) + 1)?.toUByte() ?: 0u
            ...
            result.onSuccess { voices.add(it) }
